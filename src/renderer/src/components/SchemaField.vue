<template>
  <div class="schema-field" :class="{ 'is-nested': field.type === 'nested-object' }">
    <template v-if="field.type === 'nested-object'">
      ...
      <Transition name="nested-collapse">
        <div v-if="!isNestedCollapsed" class="nested-shell">
    ...
    <template v-else>
      ...
      <div class="field-control">
        ...
        <div v-else-if="field.type === 'enum-array'" class="schema-enum-list">
          <label v-for="opt in field.options" :key="opt" class="schema-enum-item">
            <Checkbox
...
import type { SchemaField } from '../composables/useSchemaParser.js'
...
.schema-field:not(.is-nested):last-child {
  border-bottom: none;
}

.nested-content > .schema-field:last-child {
  margin-bottom: 0;
}

.nested-content > .schema-field:last-child:not(.is-nested) {
  border-bottom: none;
}

.schema-field:not(.is-nested) .field-label {
  width: 160px;
  flex-shrink: 0;
  padding-top: 8px;
  margin-bottom: 0;
